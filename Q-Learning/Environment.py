...
class Environment:
    ...
    def get_current_q_matrix(self, csv_name, n_valve=3):
        ...
        for i in range(n_valve):
            try:
                q_matrix = pd.read_csv('/home/rizky/q_learning/'+csv_name+'_'+str(i+1)+'.csv')
    ...
    def update_q_matrix(self, LH_mL):
        ...
        for j in range(len(self.Q_matrix)):
            ...
            temporal_difference = self.reward[j] + (self.disc_factor * np.max(self.Q_matrix[j][self.next_state[j]-1])) - q_value
